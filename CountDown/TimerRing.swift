        ...
        return  HStack {
            ...
            VStack(alignment: .center, spacing: 15){
                if self.Timer.isActive {
                    Text(self.Timer.isBreakActive ? "Break time 🙌" : "🔥\(self.Timer.round)")
                   //     .font(.system(size: 14))
                    .font(.custom("Dank Mono Regular", size: 12))
                } else {
                    Text(self.Timer.round > 0 ? self.Timer.isBreakActive ? "Break stopped" : "Start working" : "Create New Session")
                     //   .font(.system(size: 14))
                        .font(.custom("Dank Mono Regular", size: 12))
                        .onTapGesture {
                            if self.Timer.round == 0 {
                                ...
                                self.Timer.round = UserDefaults.standard.optionalInt(forKey: "round") ?? 5
                ...
                if self.Timer.round > 0 {
                    ...
                    Text("\(self.Timer.textForPlaybackTime(time: TimeInterval(self.Timer.timeRemaining)))")
                        ...
                        .font(.custom("Dank Mono Regular", size: 28))
